...
void CameraController::SetRightJoystickCameraSuppressed(bool suppressed) {
	if (rightJoystickCameraSuppressed == suppressed)
		return;

	rightJoystickCameraSuppressed = suppressed;
	if (suppressed) {
		// Do not let the existing smoothing/grace tail move the camera after the
		// modifier takes ownership of the right stick.
		smoothedJoystickTurnInput = 0.0f;
		lastJoystickTurnInput = 0.0f;
		joystickTurnGraceTimer = 0.0f;
	}

	uevr::API::get()->log_info("[DUALGRIP] right-stick camera %s by thumb-rest modifier",
...
void CameraController::ProcessCameraMatrix(float delta) {
	...
	if (playerManager->playerHead == nullptr)
		return;
	...
	if (rightJoystickCameraSuppressed)
		rightJoystick = {};
	...
	float rawJoystickTurnInput = 0.0f;
	if (std::abs(rightJoystick.x) > settingsManager->joystickDeadzone) {
		rawJoystickTurnInput = rightJoystick.x;
		lastJoystickTurnInput = rawJoystickTurnInput;
		joystickTurnGraceTimer = joystickTurnGraceTime;
	}
	else if (joystickTurnGraceTimer > 0.0f) {
		joystickTurnGraceTimer = (std::max)(0.0f, joystickTurnGraceTimer - delta);
		rawJoystickTurnInput = lastJoystickTurnInput * 0.65f;
	}
	else {
		lastJoystickTurnInput = 0.0f;
	}

	const float turnBlend = (std::clamp)(delta * joystickTurnSmoothingSpeed, 0.0f, 1.0f);
	smoothedJoystickTurnInput += (rawJoystickTurnInput - smoothedJoystickTurnInput) * turnBlend;
	if (std::abs(rawJoystickTurnInput) <= 0.0001f && std::abs(smoothedJoystickTurnInput) <= 0.0001f) {
		smoothedJoystickTurnInput = 0.0f;
	}

	// Joystick input to adjust the camera yaw rotation
	if (std::abs(smoothedJoystickTurnInput) > 0.0001f) {
		joystickYaw = -smoothedJoystickTurnInput * delta * settingsManager->xAxisSensitivity;
	}

	// Convert joystick yaw to radians
	float yawRadians = joystickYaw * (static_cast<float>(M_PI) / 180.0f);
...
void CameraController::ProcessHookedHeadPosition(float delta)
{
	...
	if (playerManager->isInVehicle || currentCameraMode == Fixed || keepCameraHeight)
	{
		...
		setRelativeLocation_params.newLocation = playerManager->vehicleType == 13 && playerManager->isInVehicle ?
			playerManager->defaultPlayerHeadLocalPositionUE + playerManager->defaultBikeLocalOffsetUE :
			playerManager->defaultPlayerHeadLocalPositionUE;
